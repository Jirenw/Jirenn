...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `HOLAA!! @${num.split('@')[0]} ¿COMO ESTAS?😃\n\n『BIENVENID@ *${mdata.subject}*』\n\nUn gusto Tenerte por aca UwU\n\n_Recuerda leer las reglas del grupo para no tener ningun problema y así no seas eliminad@ muy pronto 🧐_\n\n*Solo disfrutar de este grupo y divertite y bueno aporta cuando puedas UwU, y disfruta de los aportes ._. 🥳*`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Bueno, se fue @${num.split('@')[0]} 👋\n\nAdios, estoy seguro en un 99,99% de que nadie te va a extrañar :v 😎`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Espera prro estoy procesando😎\n\n❗No me salgas con spam o mi dueño te banea ctmr👏❗\n\nBy.〲⎠⎠ɪʀᴇɴ〴᭄Y🌹⃢⃟ꦼ𝔚𝔢𝔫𝔡𝔶-𝔠𝔥𝔞𝔫༒⃢⃟🇲🇽',
				success: '✔️ Listo ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa Mi dueño',
					admin: '[❗] Este comando solo puede ser utilizado por administradores del grupo',
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador',
                                        pegatina: 'Calma crack estoy haciendo tu sticker, sin hacer spam ._. 👏\n\n*Recuerda los stickersgif son de 6 segundos*\n\nBy.〲⎠⎠ɪʀᴇɴ〴᭄Y🌹⃢⃟ꦼ𝔚𝔢𝔫𝔡𝔶-𝔠𝔥𝔞𝔫༒⃢⃟🇲🇽',
...
//FUNCION ANTILINK
	...
	if (budy.includes("https://s.kwai.app/")){
		...
		reply(`*LINK DE KWAI DETECTADO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo por enviar webadas.`)
		setTimeout( () => {
	...
	if (budy.includes("https://www.facebook.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE FACEBOOK DETECTADO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo porque a nadie le interesa saber cómo estás en facebook ._.`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Adios valla a compartir su enlace a otro lado.")
		}, 0)
	}	
	...
	if (budy.includes("https://youtu.be/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE YOUTUBE DETECTADO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo, porque a nadie le interesa tu contenido.`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Adios Ojalá te llenen de reporte a tu canal u el canal que enviaste ._.")
		}, 0)
	}
	...
	if (budy.includes("https://www.instagram.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE INSTAGRAM DETECTADO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo por enviar Link de insta.`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Adios Fucking Shit")
		}, 0)
	}		
	...
	if (budy.includes("https://vm.tiktok.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE TIK TOK DETECTADO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo por enviar esa webada de enlace ._.`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Adios valla a seguir viendo TikToks")
		}, 0)
	}
       ...
       if (budy.includes("://chat.whatsapp.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE WHATSAPP DETECTADO 📢* ${sender.split("@")[0]} Usted será expulsado del grupo y posteriormente spameado UwU`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Adios sha vamos por ti guap@ UwU")
		}, 0)
	}
...
//FIN DE FUNCION DE LEVEL
       ...
       /******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'BoT-Anime&Mas'; if (!author) author = 'By.〲⎠⎠ɪʀᴇɴ〴᭄Y🌹⃢⃟ꦼ𝔚𝔢𝔫𝔡𝔶-𝔠𝔥𝔞𝔫༒⃢⃟🇲🇽';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
	       ...
	       case 'troleo':
               ...
               break*/
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido✅\n\nRetirando cargo como administrador, un admin sin odio no sirve para nada. @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                 case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '*A SUS ORDENES QUERID@ ADMIN🌚✅*\n\nAgregando cargo como administrador, yo no sé si se lo merece Pero bueno si usted lo decidió entonces está todo bien UwU:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`*A SUS ORDENES QUERID@ ADMIN🌚✅*\n\nAgregando cargo como administrador, yo no se si se lo merece pero bueno si usted lo decidió entonces está todo correcto UwU : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
...
console.log(e)
reply('Hubo un error intentalo nuevamente pero sin hacer spam ctmr :/')
}
break				
...
/******JUEGOS SHANDUY LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
			...
			case 'tagall':
				...
				case 'bc':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Chao, que le valla bien y espero nunca olvides que *ELLA NO TE AMA* 👋', text) // ur cods
					}, 0)
                     break
       /*case 'ownergrup':
				  case 'ownergroup':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recibido, Adiós prr@ JAJAJA 👋 :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						...
						mentions(`Pedido recibido, Sayonara Fuck 👋 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
			...
			//ANTILINKS DE REDES SOCIALES	
				...
				case 'linkgc':
					...
					yeh = `Aqui Tiene el enlace del grupo, úselo sabiamente 🤨👽\n\nhttps://chat.whatsapp.com/${linkgc}\n\nLink Del Grupo *${groupName}*`
			...
			case 'closegc':
					...
					const close = {
					text: `Grupo cerrado por el/la administrador(a) @${nomor.split("@s.whatsapp.net")[0]}\nAhora *solo los administradores* puede enviar mensajes, porque el/ella quiere😂`,
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo abierto por el/la administrador(a) @${sender.split("@")[0]}\nAhora *todos los participantes* pueden enviar mensajes, hablen ahora o callen para siempre.`,
					contextInfo: { mentionedJid: [sender] }
					}
	...
	//FIN DE SERVICIO DE MUSICA Y VIDEO			
				...
				case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Ya estas registrado, apoco ya te olvidaste :v 🧐')
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ ✅ ❭ La funcion de bienvenida esta habilitada en este grupo ahora daré la Bienvenida a cada pendej@ que se una XD')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
            ...
            break
					...
					/*case 'nsfwyuri':
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'PUTOOOOOOO'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
